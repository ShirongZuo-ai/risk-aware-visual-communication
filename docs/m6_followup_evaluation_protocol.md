...
Primary metric: simulator-ground-truth **Trajectory-Critical Obstacle Boundary Recall (TCOBR)** after reconstruction. An obstacle is critical when its frozen world AABB intersects the union of the frozen command-conditioned (planned) and state-only trajectory corridors, using the M3 corridor radius and geometry tolerance. The method-independent critical set is projected with the frozen M4 camera model and clipped polygon geometry. The clipped projection must contain at least 64 pixel centers. Its one-pixel inner boundary is the frozen filled-polygon mask minus its 3x3 erosion; at least 16 pixels on that boundary must be Canny edges in the original RGB frame converted to grayscale with thresholds 50 and 150. Reconstructed Canny edges match an original boundary edge within Chebyshev distance one pixel. An eligible obstacle is recalled at boundary-edge recall at least 0.50. Eligible and recalled obstacle-instance counts are pooled over four snapshots before the episode TCOBR ratio is formed; no eligible instance makes the primary episode result undefined.

The pre-registered primary episode contrast is command-conditioned minus state-only TCOBR, averaged with equal weight across Severe and Low budgets. Medium/High budget effects, full-frame PSNR/SSIM, complete actual bytes, ROI area, and scene-level results are secondary. Inference uses paired episodes, 10,000 scene-stratified bootstrap replicates within S1-S8, seed `20260724`, and a percentile 95% confidence interval. Support passes only when the primary interval's lower bound is above zero. Exclusions are limited to invalid evidence, no eligible critical obstacles, or a missing paired result; direction and magnitude are never exclusion criteria. The superseded 620xxx registration remains historical. The replacement study is frozen in the additive v3 manifest/lock and `docs/results/m6_multiscene_v3_preregistration.json`; it contains exactly the disjoint 630100-630803 identities and does not alter the v2 artifacts.
