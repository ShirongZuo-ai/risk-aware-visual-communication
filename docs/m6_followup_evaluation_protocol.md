...
The pre-registered primary episode contrast is command-conditioned minus state-only TCOBR, averaged with equal weight across Severe and Low budgets. Medium/High budget effects, full-frame PSNR/SSIM, complete actual bytes, ROI area, and scene-level results are secondary. Inference uses paired episodes, 10,000 scene-stratified bootstrap replicates within S1-S8, seed `20260724`, and a percentile 95% confidence interval. Support passes only when the primary interval's lower bound is above zero. Exclusions are limited to invalid evidence, no eligible critical obstacles, or a missing paired result; direction and magnitude are never exclusion criteria. The exact 32-episode matrix is frozen in `docs/results/m6_multiscene_preregistration.json` before data generation.
