...
- Froze the pre-generation M7 v1 development-corpus contract: exactly 16 new M7C1-M7C6/M7G1-M7G2 identities at seeds 710100-710801, geometry-only critical/low-risk checks, strict separation of sender inputs from evaluator-only obstacle geometry, canonical manifest/lock/preregistration, and an at-most-once no-retry batch path reusing the validated M6 lifecycle. The allocator, formal 720xxx split, and scientific effects remain unimplemented. Corpus execution is permitted only after this authority passes focused/full tests and is committed at a clean HEAD.
...
- Diagnosed the immutable attempt-002 `failed_process` timeout: Webots selected the prepared parent as its project, while the named controller existed only under the repository `simulator/controllers` tree, so no runtime artifacts were produced. New prepared packages now use a standard `worlds/` + `controllers/` Webots project, forward controller output, bind all runtime output paths, apply the frozen command schedule through the shared adapter, run through the 6.0 s frozen schedule, and request `simulationQuit` on success or controlled failure. Forty-eight focused tests use only temporary directories/fake Webots APIs/harmless child processes. The 75 s timeout and all frozen scientific definitions remain unchanged; no Webots process or new real attempt was started.
