...
def process_completed_pilot_launch(launch_spec,process_result,*,owned_output_root):
 root=Path(owned_output_root);runtime=json.loads(Path(launch_spec['runtime_config']['path']).read_text());summary=json.loads(Path(launch_spec['summary_path']).read_text())
 if not process_result.get('started') or not summary.get('success'):raise ValueError('runtime completion unavailable')
 records=[{'snapshot_id':sid,'timestamp_s':time,'raw_path':str(root/'raw'/(sid+'.rgb')),'metadata_path':str(root/'raw'/(sid+'.json'))} for sid,time in zip(summary['snapshot_ids'],summary['snapshot_times_s'])]
 snapshots=[process_and_audit_runtime_snapshot(runtime,x,owned_output_root=root,ownership_marker=launch_spec['owner_marker']) for x in records];aggregate=persist_codec_aggregate(runtime,snapshots,owned_output_root=root,ownership_marker=launch_spec['owner_marker']);completion={'runtime_config_sha256':runtime['config_sha256'],'codec_aggregate_sha256':aggregate['aggregate_sha256'],'runtime_summary_sha256':summary['summary_sha256'],'launch_id':aggregate['launch_id']};return validate_pilot_completion(launch_spec,process_result,summary,aggregate,completion,owned_output_root=root)
