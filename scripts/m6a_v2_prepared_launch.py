...
def build_prepared_launch_package(*,head,branch,attempt_id,package_root=CONTROL_ROOT/'prepared'):
 ...
 package={'schema_version':'m6a-v2-prepared-launch-package-v2','kind':'local-control-evidence-not-runtime-result','head':head,'branch':branch,'launch_id':provisional,'attempt_id':attempt_id,'identity_id':runtime['episode_id'],'scene_id':runtime['scene'],'seed':runtime['seed'],'preflight_workspace_root':str(base.resolve()),'prospective_attempt_root':str(root),'prospective_attempt_path_plan':paths,'launch_spec':spec,'path_plan':paths,'expected_evidence':{k:{'path':v,'required':True,'producer':'runtime-or-host'} for k,v in paths['artifacts'].items()},'launch_spec_sha256':spec['launch_spec_sha256'],'runtime_config_sha256':spec['runtime_config']['sha256'],'temporary_world_sha256':spec['temporary_world']['sha256'],'controller_sha256':spec['controller']['sha256'],'executable':spec['webots'],'argv_sha256':digest(spec['argv']),'manifest_sha256':runtime['v2_manifest_sha256'],'lock_sha256':runtime['v2_lock_sha256'],'planned_output_root':str(root),'authorization_generated':False,'launch_performed':False,'webots_started':False,'scientific_result':False}
...
def load_prepared_launch_package(path):
 ...
 if p.get('preflight_workspace_root')!=str(Path(path).parent.resolve()) or p.get('prospective_attempt_root')!=s['prospective_attempt_root'] or p['preflight_workspace_root']==p['prospective_attempt_root'] or Path(p['prospective_attempt_root']).exists():raise ValueError('preflight/attempt boundary')
