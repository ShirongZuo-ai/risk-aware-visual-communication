...
def build_prepared_launch_package(*,head,branch,attempt_id,package_root=CONTROL_ROOT/'prepared'):
 ...
 spec={'schema_version':'m6a-v2-production-launch-spec-v2','head':head,'branch':branch,'launch_id':provisional,'attempt_id':attempt_id,'identity':{'episode_id':runtime['episode_id'],'scene':runtime['scene'],'seed':runtime['seed']},'path_plan':paths,'owned_root':str(root),'runtime_config':{'path':str(config.resolve()),'sha256':_sha256(config)},'temporary_world':{'path':str(world.resolve()),'sha256':_sha256(world)},'controller':{'path':str(CONTROLLER_PATH.resolve()),'sha256':_sha256(CONTROLLER_PATH)},'webots':exe.__dict__,'argv':[exe.path,'--batch','--mode=fast',str(world.resolve())],'environment':{'M6A_RUNTIME_CONFIG':str(config.resolve())},'timeout_s':75,'graceful_termination_s':10,'expected':{'episodes':1,'snapshots':4,'methods':2,'budgets':4,'future_cases':32},'manifest_sha256':runtime['v2_manifest_sha256'],'lock_sha256':runtime['v2_lock_sha256'],'execution_authorized':False,'webots_started':False}
 spec['launch_spec_sha256']=digest(spec)
 package={'schema_version':'m6a-v2-prepared-launch-package-v1','kind':'local-control-evidence-not-runtime-result','head':head,'branch':branch,'launch_id':provisional,'attempt_id':attempt_id,'identity_id':runtime['episode_id'],'scene_id':runtime['scene'],'seed':runtime['seed'],'launch_spec':spec,'path_plan':paths,'expected_evidence':{k:{'path':v,'required':True,'producer':'runtime-or-host'} for k,v in paths['artifacts'].items()},'launch_spec_sha256':spec['launch_spec_sha256'],'runtime_config_sha256':spec['runtime_config']['sha256'],'temporary_world_sha256':spec['temporary_world']['sha256'],'controller_sha256':spec['controller']['sha256'],'executable':spec['webots'],'argv_sha256':digest(spec['argv']),'manifest_sha256':runtime['v2_manifest_sha256'],'lock_sha256':runtime['v2_lock_sha256'],'planned_output_root':str(root),'authorization_generated':False,'launch_performed':False,'webots_started':False,'scientific_result':False}
...
def load_prepared_launch_package(path):
 ...
 s=p['launch_spec'];validate_prospective_root(s['owned_root'],launch_id=s['launch_id'],attempt_id=s['attempt_id'])
