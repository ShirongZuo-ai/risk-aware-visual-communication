...
def load_v2_runtime_config(config):
 if {'actual_future','actual_future_trajectory','combined','combined_mask','oracle','oracle_mask'}&set(config) or config.get('protocol_version') not in {'m6a-byte-fair-v2','m6a-byte-fair-v3'} or config.get('config_sha256')!=digest({k:v for k,v in config.items() if k!='config_sha256'}):raise ValueError('invalid M6 runtime config')
