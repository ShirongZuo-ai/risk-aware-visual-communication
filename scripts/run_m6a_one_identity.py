...
def select_identity(episode_id,manifest_path=MANIFEST_PATH,lock_path=LOCK_PATH,*,required_split=None):
 _,data=load_and_validate_m6a_manifest(manifest_path,lock_path)
...
def build_one_identity_runtime_config(v2_manifest_path=MANIFEST_PATH,v2_lock_path=LOCK_PATH,*,output_root,episode_id='m6a_pilot_s1_seed600100'):
 authority,data=load_and_validate_m6a_manifest(v2_manifest_path,v2_lock_path);_,record=select_identity(episode_id,v2_manifest_path,v2_lock_path);root=Path(output_root)
 ...
 cfg={'protocol_version':data['protocol_version'],'manifest_authority_version':authority,'v2_manifest_path':str(Path(v2_manifest_path).resolve()),'v2_manifest_sha256':hashlib.sha256(Path(v2_manifest_path).read_bytes()).hexdigest(),'v2_lock_path':str(Path(v2_lock_path).resolve()),'v2_lock_sha256':hashlib.sha256(Path(v2_lock_path).read_bytes()).hexdigest(),'v1_supersedes_manifest_sha256':data.get('supersedes_manifest_sha256',data.get('parent_manifest_sha256')),'source_record_sha256':record['source_record_sha256'],'split':record['identity']['split'],'scene':record['identity']['scenario_id'],'episode_id':record['identity']['episode_id'],'seed':record['identity']['seed'],'snapshots':[{'snapshot_id':str(i),'timestamp_s':float(t)} for i,t in enumerate(record['snapshot_aligned_times_s'])],'schedule':schedule,'schedule_sha256':digest(schedule),'projection_config':record['projection_config'],'projection_config_sha256':record['projection_config_sha256'],'source_world':record['source_world_path'],'source_world_sha256':record['source_world_sha256'],'methods':record['methods'],'budgets':record['budgets'],'expected_snapshots':4,'expected_methods':2,'expected_budgets':4,'expected_future_cases':32,'controller':'m6a_trusted_runtime','robot_def':'ROBOT','camera':'camera','left_motor':'left wheel motor','right_motor':'right wheel motor','output_root':str(root),'actual_future_prohibited':True,'combined_mask_prohibited':True}
 cfg['config_sha256']=digest(cfg);return cfg
def load_v2_runtime_config(config):
 if {'actual_future','actual_future_trajectory','combined','combined_mask','oracle','oracle_mask','evaluator_only_obstacle_geometry','tcobr_annotations','critical_event_labels'}&set(config) or config.get('protocol_version') not in {'m6a-byte-fair-v2','m6a-byte-fair-v3','m7-development-corpus-v1'} or config.get('config_sha256')!=digest({k:v for k,v in config.items() if k!='config_sha256'}):raise ValueError('invalid M6 runtime config')
 authority,data=load_and_validate_m6a_manifest(Path(config['v2_manifest_path']),Path(config['v2_lock_path']));_,record=select_identity(config.get('episode_id'),Path(config['v2_manifest_path']),Path(config['v2_lock_path']))
 expected_snapshots=[{'snapshot_id':str(i),'timestamp_s':float(t)} for i,t in enumerate(record['snapshot_aligned_times_s'])]
 if (config.get('manifest_authority_version','v2')!=authority or config.get('protocol_version')!=data['protocol_version'] or config.get('v2_manifest_sha256')!=hashlib.sha256(Path(config['v2_manifest_path']).read_bytes()).hexdigest() or config.get('v2_lock_sha256')!=hashlib.sha256(Path(config['v2_lock_path']).read_bytes()).hexdigest() or config.get('source_record_sha256')!=record['source_record_sha256'] or config.get('identity') is not None or config.get('split')!=record['identity']['split'] or config.get('scene')!=record['identity']['scenario_id'] or config.get('episode_id')!=record['identity']['episode_id'] or config.get('seed')!=record['identity']['seed'] or config.get('snapshots')!=expected_snapshots or config.get('schedule')!={'schedule_id':record['identity']['episode_id']+'-pre-run','available_time_s':0.0,'segments':record['schedule']} or config.get('schedule_sha256')!=digest(config['schedule']) or config.get('projection_config')!=record['projection_config'] or config.get('projection_config_sha256')!=record['projection_config_sha256'] or config.get('source_world')!=record['source_world_path'] or config.get('source_world_sha256')!=record['source_world_sha256'] or config.get('methods')!=record['methods'] or config.get('budgets')!=record['budgets'] or config.get('actual_future_prohibited') is not True or config.get('combined_mask_prohibited') is not True):raise ValueError('M6 source mismatch')
