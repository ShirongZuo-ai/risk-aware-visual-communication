...
class T(unittest.TestCase):
 def test_build_and_materialize(self):
  with tempfile.TemporaryDirectory() as d:
   root=Path(d);cfg=build_one_identity_runtime_config(output_root=root/'preflight_output');p=materialize_runtime_config(cfg,root/'runtime.json');self.assertEqual(load_v2_runtime_config(json.loads(p.read_text()))['episode_id'],'m6a_pilot_s1_seed600100')
