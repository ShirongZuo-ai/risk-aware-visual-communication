    ...
    def test_unsigned_request_roundtrip_readiness_and_tamper(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("scripts.m6a_v2_execution_safety.PILOT_ROOT", root / "pilot"):
                ...
                request_path = root / "unsigned_request.json"
                ...
                self.assertTrue(load_execution_authorization_signing_request(request_path, package_path=package_path, preflight_path=package["preflight_report_path"], trust_config_path=config, repository_root=root)["signature_absent"])
                request_path.unlink()
                readiness = run_production_authorization_readiness(package_path, package["preflight_report_path"], config, request_path, repository_root=root)
