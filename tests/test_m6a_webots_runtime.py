...
class RuntimeTests(unittest.TestCase):
 def make(self,root,times=(0.,1.,2.,3.),available=0.):
  ...
  schedule=ScheduleEvidence('p',available,(CommandSegment(0,2,1,1),))
 ...
 def test_mocked_episode_writes_four_trusted_snapshots(self):
  with tempfile.TemporaryDirectory() as root:
   c,f,s,frame,q=self.make(root);summary=run_m6a_webots_episode(c,f,state_reader=s,frame_reader=frame,predefined_schedule=q)
